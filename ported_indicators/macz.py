...
def macz(candles: np.ndarray, fastperiod: int= 24, slowperiod: int=32, signalperiod:int = 8,lengthZ:int=20,lengthSTD:int=25, source_type: str = "close", sequential: bool = False) -> MACZ:
    ...
    hist = macz_t - signal
    if sequential:
        signals = _signals(source,signal,macz_t)
    else:
        signals = np.ones(source.shape[0])
    if sequential: 
        return MACZ(hist,signal,macz_t,signals)
    else:    
        return MACZ(hist[-2:],signal[-2:],macz_t[-2:],signals[-2:])
...
@njit
def _signals(source,signal,macz):
    signal = np.zeros(source.shape[0])
    for i in range(source.shape[0]):
        if macz[i] > signal[i] and macz[i-1] < signal[i-1]:
            signal[i] = 1 
        elif macz[i] < signal[i] and macz[i-1] > signal[i-1]:
            signal[i] = -1
        else:
            signal[i] = 0
    return signal
