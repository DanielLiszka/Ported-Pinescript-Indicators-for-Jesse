...
def fast_rsi(source,length):
    ...
    for i in range(source.shape[0]):
        ...
        # if np.isnan(sumation1[i]):
            # cumsum = numpy.cumsum(numpy.insert(u, 0, 0))
            # sumation1[i] = (cumsum[length:] - cumsum[:-length]) / float(length) 
        # else: 
            # sumation1[i] = alpha * u[i] + (1 - alpha) * sumation1[i-1]
        # if np.isnan(sumation2[i]):
            # cumsum2 = numpy.cumsum(numpy.insert(d, 0, 0))
            # sumation2[i] = (cumsum2[length:] - cumsum2[:-length]) / float(length) 
        # else:
