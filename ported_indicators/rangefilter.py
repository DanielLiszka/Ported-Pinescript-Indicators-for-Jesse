...
RangeFilter = namedtuple('RangeFilter',['topdev', 'bottomdev', 'filt','barcolor'])

def rangefilter(candles: np.ndarray, av_n:bool=False,av_samples:int=2, movement_source:str='close', range_mult: float = 2.6, range_period: int = 14, scale: str = "Average Change", smooth: bool = False, smoothing_period: int = 27, f_type: str = "Type 1", source_type: str = "close", sequential: bool = False) -> RangeFilter:
    """"
    Type 2 is close to accurate if provided enough candle data. ex: candles = candles[-980:]
    """
    candles = candles[-980:] if not sequential else candles #slice_candles(candles, sequential)
    source = get_candle_source(candles, source_type=source_type)  
    tr = talib.TRANGE(candles[:,3],candles[:,4],candles[:,2])
    topdev, bottomdev, filt, barcolor = rng_filt(candles,source,movement_source, range_period, f_type, smooth, smoothing_period,scale,tr,range_mult,av_n,av_samples)  

    if sequential: 
        return RangeFilter(topdev, bottomdev, filt,barcolor)
    else:    
        return RangeFilter(topdev[-2:],bottomdev[-2:],filt[-2:],barcolor[-2:])

#jesse backtest  '2021-01-01' '2021-03-02'

@njit
def rng_filt(candles,source,movement_source, t, f_type, smooth, st,scale,tr,qty,av_n,av_samples):
    ...
    rngchange = np.full_like(source,0)
    ...
    for i in range(0,source.shape[0]):
        ...
        if f_type == "Type 2": 
            if np.isnan(rng_filt1[i-1]):
                ...
                rng_filt1[i] = rng_filt1[i-1] + (np.floor(np.abs(h[i] - rng_filt1[i-1])/r[i])*r[i]) 
            elif (l[i] <= (rng_filt1[i-1] - r[i])): 
                rng_filt1[i] = rng_filt1[i-1] - (np.floor(np.abs(l[i] - rng_filt1[i-1])/r[i])*r[i])
            else:   
                rng_filt1[i] = rng_filt1[i-1]
        ...
        if rng_filt2[i] > rng_filt2[i-1]:
            rngchange[i] = 1 
        elif rng_filt2[i] < rng_filt2[i-1]:
            rngchange[i] = -1
        else:
            rngchange[i] = rngchange[i-1] 
        if rngchange[i] == 1 and source[i] > rng_filt2[i] and source[i] > source[i-1]:
            barcolor[i] = 2 
        elif rngchange[i] == 1 and source[i] > rng_filt2[i] and source[i] < source[i-1]: 
            barcolor[i] = 1 
        elif rngchange[i] == -1 and source[i] < rng_filt2[i] and source[i] < source[i-1]:
            barcolor[i] = -2 
        elif rngchange[i] == -1 and source[i] < rng_filt2[i] and source[i] > source[i-1]:
            ...
            barcolor[i] = 0 
    return hi_band2,lo_band2,rng_filt2,barcolor
